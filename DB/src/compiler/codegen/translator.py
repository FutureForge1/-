...
class QuadrupleTranslator:
    ...
    def translate(self, quadruples: List[Quadruple]) -> List[TargetInstruction]:
        ...
        for i, quad in enumerate(quadruples):
            # 跳过单独的比较操作，它们将在FILTER中被处理
            if quad.op in ['GT', 'GE', 'LT', 'LE', 'EQ', 'NE'] and self._is_comparison_for_filter(quad, quadruples):
                print(f"翻译四元式 {i+1}: {quad} (跳过，将在FILTER中处理)")
                continue
                
            print(f"翻译四元式 {i+1}: {quad}")
    ...
    def _translate_quadruple(self, quad: Quadruple):
        ...
        if op == 'SELECT':
            self._translate_select(arg1, arg2, result)
        elif op == 'FILTER':
    ...
    def _translate_count(self, source: str, column: str, result: str):
        ...
        source_reg = self.get_or_create_register(source) if source else None
        result_reg = self.get_or_create_register(result)
        
        operands = [source_reg, column] if source_reg else [column]
