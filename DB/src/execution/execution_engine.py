...
class ExecutionEngine:
    ...
    def execute(self, instructions: List[TargetInstruction]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while instruction_pointer < len(instructions):
                ...
                print(f"[{instruction_pointer:2d}] {instruction.op.value:<8} {' '.join(instruction.operands)}")
                ...
                if handler:
                    ...
                    print(f"  ⚠️  未知指令类型: {instruction.op}")
    ...
    def _execute_project(self, instruction: TargetInstruction) -> None:
        ...
        # 获取操作数，可能是多个列名
        if len(instruction.operands) >= 2:
            # 第一个操作数是源寄存器，第二个以后是列名
            columns = instruction.operands[1:]
        else:
            columns = instruction.operands
        ...
        self.context.projected_columns = columns
        
        print(f"  → 投影列: {', '.join(columns)}")
    ...
    def _execute_count(self, instruction: TargetInstruction) -> None:
        ...
        if column == '*':
            ...
            count = sum(1 for record in records if column in record and record[column] is not None)
        
        # 将结果存储在寄存器中
        if instruction.result:
            self.context.registers[instruction.result] = count
    ...
    def _execute_sum(self, instruction: TargetInstruction) -> None:
        ...
        if instruction.result:
            self.context.registers[instruction.result] = total
    ...
    def _execute_avg(self, instruction: TargetInstruction) -> None:
        ...
        if instruction.result:
            self.context.registers[instruction.result] = avg
    ...
    def _execute_max(self, instruction: TargetInstruction) -> None:
        ...
        if instruction.result:
            self.context.registers[instruction.result] = max_value
    ...
    def _execute_min(self, instruction: TargetInstruction) -> None:
        ...
        if instruction.result:
            self.context.registers[instruction.result] = min_value
