    ...
    if instructions:
        ...
        for i, inst in enumerate(instructions):
            inst_data.append({
                '序号': i + 1,
                '指令': inst.op.value,
                '操作数': ' '.join(inst.operands) if inst.operands else '-',
                '结果': inst.result or '-',
                '注释': inst.comment or '-'
            })
...
def main():
    ...
    example_queries = {
        ...
        "复杂查询": "SELECT COUNT(*) FROM students;",
    }
            ...
            if show_details:
                ...
                with st.container():
                    try:
                        ...
                        if sql_type == "SELECT":
                            # SELECT查询使用原有分析器
                            analyzer = SemanticAnalyzer() if sql_type != "COMPLEX" else ExtendedSemanticAnalyzer()
                        else:
                            # DDL/DML使用新的分析器
